...
comprehension_list_with_conditional=[]
comprehension_list_with_conditional = [numbers * 2 for numbers in range(10) if numbers % 2 == 0]
print(comprehension_list_with_conditional)
...
for number in numbers:
    if number > 0:
        for i in range(2, int(number/2)+1): #formual found online
            if (number % i) == 0:
                break
            else:
                primes.append(number)
                break  #if it is a prime number then add it.  dont sort through the ranges so break out of loop
